...
/**
 * Fetches the launcher update channel file (electron-updater generic provider
 * on R2) and surfaces the latest published version. The response is YAML, not
 * an Atom feed — only the simple top-level `version:` field is read here.
 * Full release notes live on the FoundryMTR changelog.
 */
function populateReleaseNotes(){
    $.ajax({
        url: 'https://files.foundrymtr.com/launcher/releases/win/latest.yml',
        dataType: 'text',
        success: (data) => {
            const current = remote.app.getVersion()
            const m = /^version:\s*(.+)\s*$/m.exec(data)
            const latest = m != null ? m[1].trim() : null

            if(latest != null){
                settingsAboutChangelogTitle.innerHTML = latest === current
                    ? `FoundryMTR Launcher v${current} (current)`
                    : `FoundryMTR Launcher v${latest} available`
                settingsAboutChangelogText.innerHTML = 'Full release notes are published on the FoundryMTR changelog.'
                settingsAboutChangelogButton.href = 'https://foundrymtr.com/changelog?category=launcher'
            } else {
